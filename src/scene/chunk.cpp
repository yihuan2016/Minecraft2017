//本段代码为迷你我的世界中的一段，用于实现地形地图管理。“我的世界”由成千上万的块构成，对地形的分析，获取块位置和实时渲染，移除显得十分重要。
// 本段代码涉及多重指针的构造与释放，涉及到opengl相关的VBO 应用。
    ...
    glm::vec4 bottomUV(2,0,2,0);
       if(type==Water){                               //water
           ...
           sideUV=glm::vec4(7,2,1,0);
            topUV=glm::vec4(7,2,1,0);                          //Grass
           bottomUV=glm::vec4(6,2,1,0);
       }
       else if(type==Snow){
